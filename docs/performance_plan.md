...
- 各ステップの実装時に `docs/treegantt_design.md` の改訂履歴へ対応バージョン（v0.2.59〜）を1行ずつ追記する。
...
### Step 2（v2.60）: childrenMap 導入で純関数を O(N²)→O(N) 化 — 状態: ✅完了（v2.60）
...
### Step 4（v2.62）: GanttBar / GanttLeftRow の React.memo 化 — 状態: ✅完了（v2.62）

> 実装メモ: clipPath id 重複は調査の結果、同一タスクが親バーと葉バーを同時描画することがなく実害なしのため変更せず。代わりに `assigneeOptions` 配列と `range.min/max` Date の参照安定化（値が同じなら前回参照を維持）を追加実施（これらが全行 props のため memo 無効化の原因だった）。
...
### Step 6（v2.64）: 行仮想化（自前スクロール範囲スライス） — 状態: ✅完了（v2.64）
...
### Step 7（v2.65）: バックエンド DB バッチ化 — 状態: ✅完了（v2.65）
...
## 6. 将来の拡張候補（Phase 2・未着手）

Phase 1（Step 1〜9、v2.59〜v2.67）は 2026年6月12日に全完了。以下はアーキテクチャ・設計レベルの追加施策の候補。**現時点では着手しない**。1000件シードでの実測（§4）を行い、まだ重い操作が特定された場合、または運用規模が下表の「効く規模」に達した場合に検討する。

### 候補一覧

| 施策 | 効く規模 | コスト | 着手の目安 |
|---|---|---|---|
| P2-1: 完了タスクのアーカイブ + 差分同期API | 5,000件〜 | 中 | 完了タスクが溜まり始めたら |
| P2-2: fractional ordering（疎な順序キー） | 並べ替え頻度が高い運用 | 中〜大 | 並べ替えのもたつきが報告されたら |
| P2-3: ストア正規化（Map\<id, Task\>） | 10,000件〜 | 中 | 実測で upsert の O(N) が見えてきたら |
| P2-4: SVG → Canvas 描画 | 10,000件＋高fps要件 | 大 | 最終手段。他の全候補の後 |

### P2-1: 完了タスクのアーカイブ + 差分同期API

本アプリの本質的制約は「ガントは全件をメモリに持って集計する」こと（親子進捗・CPM・イナズマラインは全件必要）。描画は仮想化済みでも、計算とメモリは件数に比例する。

- **アーカイブ**: DONE かつ終了日が古いタスクに `archived` フラグを付け、一覧APIのデフォルトから除外（必要時のみ `?includeArchived=1`）。生きているタスクは全体の2〜3割が通例のため、体感件数を恒常的に1/3にできる最有効施策。
- **差分同期**: reload 時の全件再取得を `updated_at > ?`（または単調増加バージョン番号）の差分取得に変更。再接続・インポート後の転送が数件分で済む。

### P2-2: fractional ordering（疎な順序キー）

現在の `order` は整数連番のため、1行のドラッグ移動で全タスクの order 振り直し＋全件 `tasks_reordered` ブロードキャスト（1000件で約50KB）が発生する（Step 7 で「現状維持」とした箇所）。order を間に挿入可能な疎なキー（fractional indexing）にすれば、移動は該当1件の更新・通知のみになり、並べ替えコストの規模依存が消える。代償: キーのリバランス処理、`#`（seq）とは別物だが表示系・既存テストへの影響が広い。

### P2-3: ストア正規化

`tasks: Task[]` を `Map<id, Task>` ＋順序配列に正規化し、upsert / find を O(N)→O(1) 化。`filterTasks` 以下のパイプラインが配列前提のため変更範囲が広く、1000件では体感差なし。1万件が現実になってから。

### P2-4: SVG → Canvas 描画

可視範囲の描画コストは仮想化で一定化済み。それでも「1万件＋ズームアウト全期間表示＋60fpsドラッグ」級の要件が出た場合の最終手段としてバー描画の Canvas 化がある。DOM/SVGノードは消滅するが、ヒットテスト・ツールチップ・テスト戦略の全面再構築が必要で、改修コストは Phase 1 全体より大きい。

### 検討済み・不採用

- **SQLite からクライアントサーバー型DBへの移行**: better-sqlite3（同期・単一プロセス）は本規模・同時10人に最適。移行はむしろレイテンシ悪化。
- **CRDT / Y.js への回帰**: v1.9 で廃止した経緯の通り、サーバー権威＋broadcast の現設計が適合。
- **集計計算の Web Worker 化**: 集計は O(N) 化済みで1000件なら1ms未満。ワーカー転送コストの方が高い。
