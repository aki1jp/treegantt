...
### Step 1（v2.59）: 1000件ロード保証 + 計測基盤 — 状態: 未着手

- **内容**:
  - `App.tsx` の2箇所の fetch に `?limit=10000` を明示（API 側デフォルトは変更しない＝既存 API テスト無影響）
...
- **変更ファイル**: `frontend/src/App.tsx` / `api/scripts/seed.ts`（新規）/ `frontend/src/__tests__/fixtures/genLargeTasks.ts`（新規）
- **テスト**: 初回ロード URL に `limit=` が含まれることの検証、ジェネレータの決定性・親子整合性
- **期待効果**: 1000件が実際に表示される（正しさ）。以降の計測の再現性確保
- **リスク**: 極小
...
|---|---|---|---|---|---|---|
| 1 | 2.59 | limit 明示（500件切り捨て修正）+ シード + フィクスチャ | FE+API | 正しさ（必須） | 極小 | 未着手 |
