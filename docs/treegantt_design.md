...
**高さ上限と縦スクロール**: 担当者が多くてもガント本体を覆わないよう、**行表示領域の高さを上限（`MAX_VISIBLE_ROWS` 行ぶん）にキャップ**する。上限を超える担当者は**縦スクロール**で閲覧する。ヘッダー（タイトル/凡例・日付）は固定し、左の担当者ラベル列と右のセル列の**縦スクロールを同期**（左列に可視スクロールバー、右セル上のホイールも同じ縦位置へ反映）。横スクロールは従来どおりガントと同期。
...
**ドリルダウン**: 需要のあるセルに `title` ツールチップを付け、寄与タスク名・稼働率%・その日の需要を示す。
