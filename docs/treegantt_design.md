...
| 2.39 | 2026年6月 | マイルストーン強調 UI 改善。①ガントバーのひし形アイコンに `milestoneColor` prop（`milestoneHighlightColor` を渡す）を追加し、列背景と同系色で描画。②マイルストーンラベル表示をヘッダー独立行から廃止し、最終ヘッダー行内に `data-milestone-marker` 付きの `position:absolute` オーバーレイとして統合。`MILESTONE_HEADER_H`・`milestoneHeaderH` を削除。`totalHeaderH = headerRows.length * HEADER_ROW_H + 2` に簡略化。 |
