...
│   └── package.json          version=1.4.1（/health が返す）
├── frontend/                 React SPA
│   ├── src/
...
| `BACKUP_INTERVAL_HOURS` | 24 | バックアップ間隔（時間）。`0` で無効化 |
...
- **製品バージョン**：セマンティック。現行リリース = **1.4**。`api`/`frontend` の `package.json` `version` を単一の出典とし、ハンバーガーと `/health` に表示。
