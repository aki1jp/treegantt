...
| コンポーネント | 責務 |
|--------------|------|
| `Toolbar` | **2段レイアウト**。行1（常時表示）: 検索ボックス（タイトル/担当者インクリメンタル検索）・フィルタドロップダウン（ステータス/優先度/担当者）・タスク追加・マイルストーン追加・Import/Export・∧/∨折りたたみトグル。行2（∧/∨で折りたたみ可・状態はlocalStorage永続化）: ズーム選択・ガント開始日・期間・ヘッダー行トグル（年/月/週/日）・イナズマラインON/OFF・土日強調・クリティカルパスCP・**担当者ビューON/OFF**・文字サイズ・行高。テーマ選択はプロジェクトヘッダー右端に配置 |
