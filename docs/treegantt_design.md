...
│   └── package.json          version=1.2.2（/health が返す）
├── frontend/                 React SPA
│   ├── src/
...
- **不変条件**：`resolveVisibleId` が使う行位置の索引（`taskIndex`）と、解決後にタスク実体を引く ID→タスク の Map（`taskById` 相当）は、**必ず同じ可視集合**（フィルタ結果ではなく画面に表示される行の集合＝`flatRows`）から構築する。索引側だけがフィルタ結果より広い集合（表示専用祖先を含む）から作られ、実体引き当て側がフィルタ結果のみから作られていると、可視祖先に解決された ID が実体 Map に存在せず `undefined` になり得る。呼び出し側での存在チェックを省略するとレンダー中に例外となり、本アプリには全体を捕捉する ErrorBoundary（§7.4）がないため白画面化する。
...
| 0.2.123 | 2026/7 | フィルタ後ツリーの祖先再表示（`includeAncestors`、構造保持のための表示専用祖先）を担当者フィルタの親子継承とは別メカニズムとして明記（§9.3）。依存矢印の可視祖先解決（`resolveVisibleId`）はこの表示専用祖先も含めて扱う旨、および行位置索引とタスク実体引き当ては必ず同じ可視集合から構築するという不変条件を明記（§8.5）。全体を捕捉する `ErrorBoundary` を新設し、想定外の描画エラー時に白画面ではなくフォールバック UI（再読み込み案内）を表示する方針を追加（新設§7.4、§7.2 コンポーネント表）。 |
