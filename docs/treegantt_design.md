...
│   └── package.json          version=1.8.0（/health が返す）
├── frontend/                 React SPA
│   ├── src/
...
UI 表示言語を日本語/英語で切り替えられるようにする。新規ライブラリ（`react-i18next` 等）は追加せず、既存の依存を最小限（`dayjs`/`papaparse`/`react`/`react-dom`/`react-markdown`/`remark-gfm`/`zustand`）に保つ方針のまま、軽量な自前実装で対応する。**段階導入**：主要画面（基盤・ツールバー・`App.tsx` 本体・主要モーダル・WBS/ガント本体）を第1弾として対応し、コンテキストメニュー・リソースビュー・プロジェクト管理・細かいトースト文言・APIエラーコードの全件マッピングは今後の対象（未対応の画面は日本語表示のまま残る）。
...
- **製品バージョン**：セマンティック。現行リリース = **1.8**。`api`/`frontend` の `package.json` `version` を単一の出典とし、ハンバーガーと `/health` に表示。
