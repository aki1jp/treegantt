...
| 2.75 | 2026年6月 | セキュリティ: `fastify` を v4→v5 メジャー更新し HIGH 5件を解消。残存していた HIGH（`fastify` の sendWebStream DoS・Content-Type タブ文字での body 検証バイパス・X-Forwarded-Proto/Host 偽装、推移依存の `fast-uri` パストラバーサル・`fast-json-stringify`・`@fastify/ajv-compiler`）はアドバイザリ範囲が `fastify <=5.8.2` で 4.x にパッチ版が無く、`fastify@^5.8.5` への更新が必須だった。連動して `@fastify/cors`(^9→^11)・`@fastify/compress`(^7→^9) も fastify5 対応ラインへ更新。Node は `node:20-slim` で fastify5 の Node20+ 要件を満たす。コード変更は不要（`index.ts` の register/`setErrorHandler`/`listen`、`auth.ts` の preHandler hook＋型拡張、`compression.ts` の compress オプション、各ルートの JSON スキーマはいずれも v5 互換）。`security.test.ts` に fastify/cors/compress の major 下限ガードを追加し、回帰は既存の `routes`/`adversarial`/`compression` テスト（`.inject()` で実スタック通過）でカバー。残る `uuid` MODERATE は別途対応。API・表示仕様・ストア変更なし。 |
| 2.76 | 2026年6月 | 本番ビルド復旧: 既存のテストフィクスチャ漏れを修正。`api/src/__tests__/sort.test.ts` の `makeTask` が `Task` 型に必須の `titleColor`/`titleBgColor`（タスクカラー機能で追加済み）を欠いており、`tsc` が型エラーを出して本番 Docker ビルド（`npm run build`＝`tsc`）が失敗していた。両フィールドに `null` を補完。`vitest` は型チェックしないため `npm test` では従来検知できなかった潜在不具合。テスト・API ロジック・表示仕様・ストア変更なし（フィクスチャの型整合のみ）。 |
| 2.77 | 2026年6月 | セキュリティ: 最後の脆弱依存 `uuid`（MODERATE: `buf` 提供時のバッファ境界チェック欠如 GHSA-w5hq-g745-h8pq）を `^10`→`^11.1.1` に更新し `npm audit` をクリーン化（脆弱性 0 件）。使用は `v4` 名前付きインポートのみ（`projectService`/`taskService`/`routes/tasks`/`routes/importExport`）で API 互換。`uuid` v11+ は型定義を同梱するため不要になった `@types/uuid`（devDependency）を削除。`security.test.ts` に `uuid` major>=11 ガードを追加。実害は元々なし（`buf` 未使用）だが残存 MODERATE を解消。API・表示仕様・ストア変更なし。 |
| 2.78 | 2026年6月 | セキュリティ仕上げ: dev 専用の推移依存 `esbuild`（HIGH、`tsx`・`vitest`→`vite` 経由）を `npm audit fix` の非破壊 patch 更新（0.28.0→0.28.1）で解消し、API の `npm audit` を **dev 含め完全0件**に。該当アドバイザリ（Deno での RCE / Windows dev サーバーのファイル読取）は本環境では非該当・本番イメージにも非同梱（`npm prune --omit=dev`）だが、監査クリーン維持のため対応。`package-lock.json` のみの変更（直接依存・ソース変更なし）。回帰は全テストで確認。 |
