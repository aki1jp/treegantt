...
- **GitHub Actions CI**（`.github/workflows/ci.yml`）：push/PR で `api`・`frontend` それぞれ `npm ci` → `npm run typecheck` → `npm run lint` → `npm test` を実行。E2E（Playwright）は対象外（別途手動/将来検討）。
- 既知の残課題（未着手）：Prettier 導入、カバレッジ閾値のゲート化、CI での `npm audit`、CI への E2E(Playwright) 組み込みは 17.1 に残す。
...
### 17.1 自動ゲート（最優先・再発防止）
CI・ESLint・`typecheck` npm script は **導入済み**（16.5 参照）。残る検討候補は以下。
