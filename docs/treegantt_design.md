...
| コンポーネント | 責務 |
|--------------|------|
| `Toolbar` | **検索ボックス**（最左端・タイトル/担当者インクリメンタル検索）・フィルタ（ステータス/優先度/担当者ドロップダウン）・ズーム選択・ガント期間コントロール・イナズマラインON/OFFボタン・クリティカルパス「CP」トグルボタン・ガントヘッダー行トグルボタン（年/月/週/日）・Import/Export・タスク追加ボタン |
