...
### 9.2 WBS とインライン編集
- 列：No.・#（seq）・タイトル・担当者・進捗・開始日・終了日・期間。`wbsHiddenCols` で表示列を制御、`wbsPanelOpen` で WBS パネル開閉。
- **No. 列（表示専用の通し番号）**：既存の「#」列（`task.seq`＝作成時発番の不変 ID）とは別に、上から何行目かを示す表示専用の連番列を「#」列の直前（先頭）に配置する。基準は「全展開（折りたたみなし）・フィルタなし」の表示順で固定し、`GanttChart.tsx` の `displayTasks`（クロスプロジェクト参照合成後の一覧）を対象に `buildTree` → `flattenTree(roots, 空の Set)` でツリー順にフラット化した順序で 1 から採番する。実際の画面（フィルタ適用後・折りたたみ適用後）では、表示されている各行に対してこの基準で決まった元の番号をそのまま表示し、行が非表示になっても残った行の番号を詰め直すことはない（例：元 1,2,3,4,5 のうち 2 番の行が非表示になった場合、残りは 1,3,4,5 のまま表示される）。クロスプロジェクト参照の合成グループ行（`ref:<projectId>`）および参照タスク本体も `displayTasks` に含まれるため、同じ通し番号の対象となる。`wbsHiddenCols`（`WbsPanel.tsx` の `HIDEABLE_COLS`）には含めず、「#」列と同様に常時表示とする。
