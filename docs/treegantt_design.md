...
### 2.1 アーキテクチャ全体像
**2 つの OS プロセス**（フロントエンドと、REST API・WebSocket を単一 Node プロセスで提供する API プロセス）から成る。下表は層としては 3 つに分けて記載するが、REST と WebSocket は同一 Node プロセス（`index.ts`）が 2 つのポートで待ち受けているだけで、別プロセスではない。

| 層 | 技術 | 既定ポート | 役割 |
|----|------|-----------|------|
| フロントエンド | React 18 + TypeScript + Vite | 3000 | SPA。ガント UI・状態管理 |
| REST API | Fastify 5 + better-sqlite3 | 4000 | CRUD・Import/Export・/health |
| WebSocket | `ws` ライブラリ（REST と同一 Node プロセス・別ポート） | 4001 | プロジェクト単位の broadcast |
...
│   │   │   ├── backup.ts      定期バックアップ（起動時+interval、世代管理）§13.4
...
│   └── package.json          version=1.9.4（/health が返す）
├── frontend/                 React SPA
│   ├── src/
...
├── mcp/                      AI連携用 MCP サーバー（api/frontendとは独立。詳細は §18）
│   ├── src/
│   │   ├── index.ts          McpServer 起動・ツール登録・stdio transport 接続
│   │   ├── client.ts         API_BASE_URL への fetch ラッパー
│   │   └── tools.ts          読み取り専用ツール定義（5件）
│   └── package.json
├── e2e/                       Playwright E2E
├── docs/                      本書・FEATURES.md・performance_plan.md・ai_integration_policy.md
└── docker-compose*.yml
```
...
- **既知の未検証経路**：Import（`restore`）の `predecessors` リマップ（§10、Pass 3）は `INSERT OR IGNORE INTO task_deps` への直接 INSERT であり、`wouldCreateDepCycleDb` を経由しない。そのため、循環を含む依存関係をエクスポートしたデータを再インポートすると、その循環が検証を経ずに復元されうる（本検証の導入前から存在する既知挙動であり、本節での新規デグレードではない）。
...
### 6.4 ハードニング（メッセージサイズ・オリジン検証）
認証の導入自体は Phase 2 スコープ外だが、次の2点は現行で対応する。
- **メッセージサイズ制限**：`WebSocketServer` に `maxPayload`（64KB）を設定する。subscribe メッセージしか受け付けないため十分な余裕がある。上限超過時は `ws` の既定動作でコネクションが自動切断される（close code 1009）。
- **オリジン検証**：接続確立時（`verifyClient`）に、リクエストの `Origin` ヘッダーが**存在し、かつ** `CORS_ORIGIN` 環境変数が `*` 以外に設定されている場合のみ、`Origin` が `CORS_ORIGIN` と完全一致しない接続を拒否する。`Origin` ヘッダーが無い接続（MCP・CLI 等の非ブラウザクライアント）は既存動作のまま許可する。既定（`CORS_ORIGIN` 未設定＝`*`）では挙動は変わらない。
...
- エラー通知用の非永続ストア `toastStore`（zustand）は表示中のトースト一覧（`id`/`message`/`type`）を保持する。永続化しない（画面遷移・リロードで消えてよい一過性の通知のため）。仕様は §9.9 参照。
...
| `Toast`/`ToastContainer` | エラー等の非モーダル通知（自動消滅・手動クローズ、§9.9） |
...
- 折りたたみ時は `resolveVisibleId` で可視祖先へリダイレクト。可視範囲と交差するもののみ描画。可視祖先へのリダイレクトは「親を折りたたんだことで子が隠れている」場合だけでなく、「担当者フィルタの構造保持でツリーに残った表示専用祖先」（§9.3）の場合も同じ経路で扱う。
- **不変条件**：`resolveVisibleId` は「起点/終点 ID → 可視な祖先 ID」を求めるため `parentId` を遡るが、この遡りの途中に**折りたたみで隠れているだけのタスク**を経由することがある。そのため解決後にタスク実体を引く ID→タスク の Map（`taskById` 相当）は、**画面に表示中の行（`taskIndex`／折りたたみ後の `flatRows`）だけでなく、折りたたみで隠れているタスクも含む「概念上のツリー全ノード」**（フィルタ一致タスク＋構造保持のための表示専用祖先＝`includeAncestors` の出力、折りたたみ状態には依存しない）から構築する。フィルタ結果一覧（`sorted`）だけから作ると表示専用祖先が欠落し、逆に折りたたみ後の可視行（`flatRows`）だけから作ると折りたたみで隠れた祖先チェーンの遡りに失敗する。どちらの場合も可視祖先に解決された ID が実体 Map に存在せず `undefined` になり得る。呼び出し側での存在チェックを省略するとレンダー中に例外となり、本アプリには全体を捕捉する ErrorBoundary（§7.4）がないため白画面化する。
...
### 9.2 WBS とインライン編集
- 列：行・#（seq）・タイトル・担当者・進捗・開始日・終了日・期間。`wbsHiddenCols` で表示列を制御、`wbsPanelOpen` で WBS パネル開閉。
- **「行」列（表示専用の通し番号）**：既存の「#」列（`task.seq`＝作成時発番の不変 ID）とは別に、上から何行目かを示す表示専用の連番列を「#」列の直前（先頭）に配置する。基準は「全展開（折りたたみなし）・フィルタなし」の表示順で固定し、`GanttChart.tsx` の `displayTasks`（クロスプロジェクト参照合成後の一覧）を `order` 昇順に揃えた上で `buildTree` → `flattenTree(roots, 空の Set)` でツリー順にフラット化した順序で 1 から採番する。実際の画面（フィルタ適用後・折りたたみ適用後）では、表示されている各行に対してこの基準で決まった元の番号をそのまま表示し、行が非表示になっても残った行の番号を詰め直すことはない（例：元 1,2,3,4,5 のうち 2 番の行が非表示になった場合、残りは 1,3,4,5 のまま表示される）。クロスプロジェクト参照の合成グループ行（`ref:<projectId>`）および参照タスク本体も `displayTasks` に含まれるため、同じ通し番号の対象となる。`wbsHiddenCols`（`WbsPanel.tsx` の `HIDEABLE_COLS`）には含めず、「#」列と同様に常時表示とする。列見出しの英語表記は "Row"（将来の英語表示対応時の参考。現時点で i18n 機構は未導入）。
...
- 作成ドラッグ：開始日が未設定の葉タスク（非親・非マイルストーン）の行は crosshair で、行背景をドラッグして開始/終了日を作成できる。**クリックの手ぶれによる誤作成を防ぐため、mousedown 位置から `CREATE_DRAG_THRESHOLD_PX`（既定 4px）以上ドラッグして初めて作成対象になる**（閾値未満の mouseup では日付を作らない。閾値を一度超えればその後は通常追従＝スティッキー）。アンカーセル内に収まるドラッグは 1 日タスクを作成する。土日背景・マイルストーン列背景などのオーバーレイ `<rect>` は `pointer-events: none` を付与し、行背景のマウスイベントを妨げない（土日列上でも作成ドラッグが機能する）。
...
API 呼び出し失敗などユーザーに伝えるべきエラーは、`alert()`（モーダルでブロックする）や無言の握りつぶし（`.catch(() => {})`）ではなく、**非モーダルのトースト通知**（`toastStore` + `Toast`/`ToastContainer`, `components/Toast/`）で表示する。

- **保持内容**：`toastStore`（zustand・非永続, §7.1）が表示中トースト一覧 `{ id, message, type }[]` を保持する。`type` は `'error' | 'success' | 'info'`。
- **表示位置**：画面右下に固定（`position: fixed; right; bottom`）。複数同時発生時は縦に積み上げ、新しいものが下に追加される。
- **自動消滅**：`type` ごとに既定表示時間の経過後、自動的に消える（手動操作不要）。`error`=6000ms、`success`=3000ms、`info`=4000ms。
- **手動クローズ**：各トーストに `✕`（`aria-label="閉じる"`）ボタンを持ち、即座に消せる。
- **アクセシビリティ**：コンテナに `role="status"` `aria-live="polite"` を付与し、スクリーンリーダーにも通知内容が伝わるようにする。
- **適用範囲**：
  - `App.tsx` の各種 CRUD 失敗（保存・更新・削除・コピー・プロジェクト作成/改名/削除・設定保存）と `useImportExport` の Import 失敗：従来の `alert('...失敗しました: ' + err.message)` をエラートーストに置換。
  - 初回ロード（プロジェクト一覧・アプリ既定設定・プロジェクト切替時のタスク取得・`reload` イベントでの再取得）の失敗：無言の `.catch(() => {})` を撤去し、エラートーストを出す。**加えて、プロジェクト一覧の初回取得失敗、および選択中プロジェクトのタスク初回取得失敗は、画面遷移をブロックしない範囲で「取得に失敗しました」＋再試行ボタン付きの可視エラー表示に置き換える**（トーストは一過性のため、再試行導線は画面内の恒常的な表示で担保する）。バックエンドバージョン取得（`/health`、表示専用の付加情報）はエラートーストのみで再試行導線は設けない。
  - 楽観的更新（`useTasks` の `updateTask`/`deleteTask`/`reorderTasks`）がロールバックされた場合も、呼び出し元（`App.tsx`）がエラートーストで失敗を可視化する。**`GanttChart` の行 D&D（WBS 行のドラッグ&ドロップによる並び替え）失敗も対象**：`App.tsx` は `useTasks` の `reorderTasks` を `GanttChart` の `onReorder` へ直接渡さず、`handleReorder`（try/catch でロールバック後にエラートーストを出す、他の CRUD ハンドラと同じパターン）でラップして渡す。
  - **`confirm()`/`prompt()`（削除確認・プロジェクト作成/改名などの対話 UI）は今回のスコープ外**でそのまま残す（トースト化しない）。

### 9.10 アクセシビリティ（基本方針）

スクリーンリーダー等の支援技術で操作可能にするための最低限の方針。**キーボード操作の完全対応（D&D のキーボード代替、フォーカストラップ）は今回のスコープ外**で、§17.2 に将来課題として残す。
...
UI 表示言語を日本語/英語で切り替えられるようにする。新規ライブラリ（`react-i18next` 等）は追加せず、既存の依存を最小限（`dayjs`/`papaparse`/`react`/`react-dom`/`react-markdown`/`remark-gfm`/`zustand`）に保つ方針のまま、軽量な自前実装で対応する。**段階導入**：第1弾（基盤・ツールバー・`App.tsx` 本体・主要モーダル・WBS/ガント本体）に続き、第2弾でコンテキストメニュー・ダイアログ/トースト・リソースビュー・プロジェクト管理/クロスプロジェクト参照管理・hooks 各所のトースト文言・API エラーコードの主要マッピングに対応し、フロントエンドの静的 UI 文言はおおむね翻訳対象を網羅した。エクスポート/インポート時のタイトル複製サフィックス（`(コピー)`/`(Copy)`）や今日ラインの時刻表示（`toLocaleTimeString`）などロケール依存の細部も含む。API が返すエラーメッセージのうち未マッピングの `code`/想定外エラーは引き続き英語表示のまま残る。

- **辞書**：`frontend/src/i18n/ja.ts`（フラットな `Record<string, string>`、キーは `'toolbar.addTask'` のようなドット区切りの名前空間文字列）と `frontend/src/i18n/en.ts`。`en.ts` は `export const en = { ... } satisfies typeof ja;` として TypeScript の `satisfies` でキーの完全一致を強制し、キー漏れ・タイポはビルドエラーで検出する。
- **フック**：`frontend/src/i18n/useTranslation.ts` が `taskStore` の `locale` を読み、`t(key, vars?)` を返す。`vars` は `{name}` 形式の単純な文字列置換（タスクタイトル・件数・フィールド名などの動的埋め込みに対応）。
- **状態と永続化**：`taskStore.ts` に `locale: 'ja' | 'en'`（既定 `'ja'`）と `setLocale` を追加し、`theme`（§9.6）と同じ `partialize` リストに含めて localStorage に永続化する。**既定値 `'ja'` により、既存ユーザーの見た目は変更されない。**
- **切替UI**：`App.tsx` の既存テーマ切替ボタン（☀🌙🖥）の隣に「日本語 / English」のテキストトグルを配置する（絵文字の国旗は言語と国の対応が不正確なため使わない）。
- **翻訳対象**：静的UI文言全般（ラベル・ボタン・メニュー・ツールチップ・`aria-label`・`placeholder`・トースト・確認ダイアログ・列見出し・曜日表示・バリデーションメッセージ）。
- **翻訳対象外**：ユーザーが入力したデータ（タスクタイトル・説明・担当者名・プロジェクト名）は翻訳しない。CSV の列見出しは既に英語の内部フィールド名（`title`/`assignee` 等）でありエクスポート/インポート仕様への影響はない。
- **APIエラーメッセージ**：バックエンドは英語固定文字列＋`code`（例 `DEP_CYCLE_DETECTED`）を返すが、`code` は未使用でエラー文自体をそのまま連結表示していた（i18n 導入前から日英混在）。`utils/api.ts` の `apiFetch` を拡張し `code` を保持するエラー型（`ApiError extends Error { code?: string }`）にし、辞書の `apiError.*` に既知の `code` を翻訳登録する。未マッピングの `code`/想定外エラーは元の英語メッセージにフォールバックする（バックエンド自体は変更しない）。
- **重複解消**：曜日ラベル配列 `DOW_LABELS`（`utils/ganttCalc.ts` と `ResourceSettingsModal.tsx` に重複定義）を一本化する。曜日は添字アクセス（`DOW_LABELS[dow]`）が必要でフラットな `t()` 辞書には向かないため、専用モジュール `i18n/dow.ts`（`DOW_LABELS_JA`/`DOW_LABELS_EN` の2配列＋ロケールに応じた配列を返す `useDowLabels()` フック）として切り出す。`ganttCalc.ts` 側は純粋関数のままとし、曜日ラベル配列を引数として受け取る形にする。
- **列見出しの重複解消**：`GanttChart.tsx` の `LEFT_COLS` と `WbsPanel.tsx` の列定義配列で重複していた列ラベル文字列を、この対応にあわせて辞書へ一本化する。
- **テストへの影響**：既定ロケールが `'ja'` のままのため、既存のテスト（日本語文字列を `getByText` 等で検索する箇所）は無改修で通る。各画面の対応時に「`locale='en'` で該当文言が英語表示される」ことを確認する簡潔なスモークテストを追加する（全アサーションの英語版複製は行わない）。
...
- **CSV 式インジェクション対策**：Excel 等の表計算ソフトはセル先頭が `=` `+` `-` `@` のとき数式として解釈する（CSV Injection）。CSV Export（API `routes/importExport.ts`・フロント `exportToCsv`）では、ユーザー入力に由来する文字列列（`title`/`summary`/`description`/`assignee`）のうち先頭が `=+-@` のいずれかのセルにシングルクォート `'` を付与して無害化する（OWASP 推奨方式）。`id`/`parentId`/`status`/`priority`/`progress`/日付/`isMilestone`/`titleColor`/`titleBgColor`/`estimateMinutes`/`predecessors` など機械生成の列は対象外。CSV Import（フロント `importFromCsv`）では、対象4列の値が `'` で始まり、かつ直後の文字が `=+-@` のいずれかの場合のみ `'` を除去し、`export→import` の往復で元の値に復元する（round-trip）。API の JSON Import はそのまま（JSON 経路は式解釈の問題がない）。負の日数や「`-課題名`」のような正当な値の先頭が `-` 等と一致する場合も一時的に `'` が付与されるが、往復で復元されるため許容する。
...
- **WebSocket ハードニング**：`wsRoom.ts` に `maxPayload`（64KB）とオリジン検証を導入（詳細は §6.4）。認証・認可自体の導入は引き続きスコープ外（Phase 2）。
- **入力検証**：Fastify JSON スキーマ（enum/長さ/型）。全 SQL はパラメータ化（SQL インジェクション対策）。
- **エラーレスポンスのマスキング**：集約エラーハンドラ（`app.ts` の `setErrorHandler`）は `statusCode>=500`（未設定時は 500 扱い）のとき、レスポンスの `error` を汎用文言 `Internal Server Error` に置き換え、スタック由来の詳細等の内部情報を返さない。詳細は Fastify logger にのみ出力する。4xx（バリデーションエラー等）は従来どおり `err.message` をそのまま返す（クライアントの入力訂正に必要なため）。
- **XSS**：Markdown は `react-markdown`（生 HTML 不許可・URL サニタイズ既定）。`dangerouslySetInnerHTML`/`eval` 不使用。
- **依存セキュリティ**：`api` は `npm audit` クリーン（fastify 5 系・uuid 11 系・ws 8.21.0 以上［HIGH: メモリ枯渇 DoS GHSA-96hv-2xvq-fx4p 対応］。未使用の脆弱依存 fast-jwt は不採用）。`frontend` は vite/esbuild/undici 等 **dev 専用**依存に既知の脆弱性が残るが、本番ビルド成果物（`dist/`）には非同梱のため実害なし（件数・詳細は §16.4）。CI では `npm audit` を自動ゲート化し（§16.5）、`api` は全依存・`frontend` は `--omit=dev`（本番依存のみ）を対象に HIGH 以上で失敗させる。
- **CORS**：`CORS_ORIGIN`（既定 `*`）＋許可メソッド明示（`plugins/cors.ts` の `corsOptions`、5.1 参照）。既定 `*` は社内 LAN 運用の互換維持のため変更しない。プリフライト（OPTIONS）が PATCH/DELETE を許可することを `cors.test.ts` で回帰検証する（`app.inject()` の CRUD テストはプリフライトを経由しないため、別途プリフライトを明示的に inject する）。
  - **本番運用ガイド**：インターネット公開等、既定の全許可が不適切な環境では `CORS_ORIGIN` に具体的なフロントオリジン1つ（例: `https://gantt.example.co.jp`）を設定する。この値は REST（`@fastify/cors`）と WebSocket（§6.4 のオリジン検証）の双方に効く。開発は `.env.example` にコメントで設定例を記載、本番（Docker）は `docker-compose.yml` の該当行にコメントを付与済み（コード変更不要、`.env` で上書きするのみ）。
- **CSV**：式インジェクション（先頭 `=+-@`）は Export 時にシングルクォート付与で中和し、Import 時に往復復元する（実施済み。詳細は §10）。
...
| `BACKUP_DIR` | DB ファイルと同じディレクトリ配下の `backups/` | 定期バックアップの保存先（§13.4） |
...
- **`api` の healthcheck**：`docker-compose.yml` に既存 `/health` を叩く `healthcheck` を設定（`node -e "fetch('http://localhost:4000/health')..."` で判定。`node:20-slim` に `curl` が同梱されていないため、イメージに確実に存在する Node.js 組み込みの `fetch` を使う）。`frontend` は `depends_on.api.condition: service_healthy` で、API の起動完了（DB マイグレーション等を含む）を待ってから起動する。
...
- **方式**：`api` プロセス内で `better-sqlite3` のオンラインバックアップ API（`db.backup(destPath)`、Promise を返す）を使い、稼働中の DB から一貫性のあるスナップショット（WAL の内容を統合済みの単一ファイル）を取得する。`sqlite3` CLI は `node:20-slim` に同梱されないため使用しない。
- **タイミング**：API 起動時に1回 ＋ `BACKUP_INTERVAL_HOURS`（既定 24、`0` で無効）ごと。`api/src/index.ts` が `api/src/db/backup.ts` の `scheduleBackups()` を明示的に呼び出して起動する（`db/client.ts`・`ws/wsRoom.ts` のような import 時副作用にはしない。テスト容易性のため）。
- **保存先・世代管理**：`BACKUP_DIR`（既定：DB ファイルと同じディレクトリ配下の `backups/`）に `treegantt-<YYYYMMDDTHHMMSS>.db` の形式で保存する。`BACKUP_RETENTION`（既定 7）を超える古い世代はファイル名（＝タイムスタンプ）昇順で削除する。
- **失敗時の扱い**：バックアップ取得・削除に失敗してもプロセスを落とさず、`fastify.log.error` に記録して継続する（次回のスケジュール実行は継続される）。
- **開発／本番の両対応**：
  - 本番（Docker）：`docker-compose.yml` は `./api/data:/app/data` を丸ごとボリュームマウントしており、`DB_PATH=/app/data/treegantt.db` のため既定の `BACKUP_DIR`（DB と同じディレクトリ配下）は自動的に `/app/data/backups/` となり、ホスト側 `./api/data/backups/` に永続化される。**`docker-compose.yml` の変更は不要。**
  - 開発（`start.sh`）：`DB_PATH` 未設定時の既定は `api/data/treegantt.db`（`db/client.ts`）で、本番と同じディレクトリ構造のため既定の `BACKUP_DIR` は `api/data/backups/` になる。**`start.sh` の変更は不要。**
  - バックアップファイルは DB ファイルと同じ拡張子（`.db`）のため、既存の `.gitignore`（`*.db`）・`api/.dockerignore`（`data`）でそのまま除外される。
- **手動リストア手順**：
  1. API プロセスを停止する（本番: `docker compose stop api`、開発: `start.sh` を Ctrl+C）。
  2. リストアするバックアップファイルを `DB_PATH` の場所へコピーする（例: `cp api/data/backups/treegantt-20260704T120000.db api/data/treegantt.db`）。
  3. 元の DB に付随する WAL/SHM ファイル（`treegantt.db-wal`/`treegantt.db-shm`）が残っていると起動時に不整合が生じ得るため、リストア前に削除しておく（`db.backup()` によるスナップショット自体は WAL 内容を統合済みの単一ファイルであり WAL/SHM を伴わない）。
  4. API を起動する。起動時のマイグレーション適用は冪等なため、リストアした DB に対して再実行されても問題ない。
- **実装**：`api/src/db/backup.ts`（`runBackup`/`pruneOldBackups`/`scheduleBackups`。いずれも DB・ディレクトリ・時刻・logger を引数（依存）として受け取り、ユニットテスト可能にする）。`api/src/config.ts` に `resolveBackupDir`/`resolveBackupIntervalHours`/`resolveBackupRetention` を追加。
...
- **製品バージョン**：セマンティック。現行リリース = **1.9**。`api`/`frontend` の `package.json` `version` を単一の出典とし、ハンバーガーと `/health` に表示。
...
### 16.4 既知の保留課題
- **frontend の dev 専用依存の既知脆弱性**：`npm audit`（全依存）に残る（現状: high 2件［vite・undici］・moderate 1件［esbuild］・low 1件［@babel/core]）。いずれも **dev サーバー/テストツールチェーン専用**で本番ビルド成果物（`dist/`）には非同梱のため実害なし。修正には vite のメジャー更新（破壊的）が必要なため、16.3 の統一に合わせて解消する。CI の `npm audit` ゲート（§16.5）は `--omit=dev` で本番依存のみを対象とし、これら dev 専用の既知項目で CI が落ちないようにしている。
- **`App.tsx` のユニットテスト**：オーケストレーション層のユニットテスト（`app.test.tsx`）を追加済み。主要な CRUD/トースト配線・行 D&D 等を検証し、カバレッジは約50%（複雑な分岐やモーダル配線は引き続き E2E で補完）。

### 16.5 自動ゲート（CI・ESLint・型チェック）
17.1 で検討課題としていた自動ゲートを導入済み。
...
- **ビジュアルリグレッションテスト**（Playwright のスクリーンショット比較、`toHaveScreenshot`）は**導入済み**。描画中心のアプリ（バー/マイルストーン/依存矢印/WBS）で意図しない見た目変化を検出する。
  - **対象範囲**（`e2e/tests/visual-regression.spec.ts`）：全画面撮影は差分ノイズで運用が破綻するため、代表的な2シナリオに絞る。①**ガントメイン表示**（`data-testid="gantt-chart-body"`＝WBS＋ガントの結合領域。親子タスク・依存矢印2本・進捗の異なるステータス混在・マイルストーン菱形を含む代表状態）②**TaskModal**（`data-testid="task-modal-panel"`。タイトル/日付/担当者/ステータス/優先度/先行タスクなど主要フィールドを埋めた編集画面）。**ツールバー**（行1操作列＋行2フィルタ/表示設定を展開した状態）はフィルタ・表示設定のラベル/ボタンが密集し文字要素の総面積が他シナリオより大きいため、CJKフォントのグリフレンダリング差（ローカルのNoto Sans JPとCIのNoto Sans CJK JP）の影響を最も受けやすく、CIでのみ閾値超過（4%）で継続的に失敗したため対象シナリオから除外した（1.9.3、`toolbar-chromium-linux.png`のベースラインも削除）。
  ...
  - **閾値**：`expect.toHaveScreenshot` の `maxDiffPixelRatio: 0.02` をグローバル既定（`playwright.config.ts`）とする。フォントのアンチエイリアス等の微小な差は許容しつつ、レイアウト崩れ等の実質的な変化は検出する値として採用した。**TaskModal のみ個別に 0.035 へ緩和**（1.9.4）：CI で毎回ほぼ同一のピクセル数（9334px・約3%）で安定して超過しており、ツールバーのような不規則な超過ではなく CJK フォント差による小さく一定した超過と判断できるため、除外ではなく `expectClippedScreenshot` の呼び出し側で `maxDiffPixelRatio` を個別指定して吸収する。
  ...
  - **寸法不一致への対処（クロップ撮影）**：`expect(locator).toHaveScreenshot()` は比較先の画像と寸法が一致しないと `maxDiffPixelRatio` を評価する前に即失敗する。環境間のフォントレンダリング差はピクセル内容だけでなく、対象要素の自然な境界サイズ（高さ）を数px揺らすことがあり（実例: CI で `data-testid="gantt-chart-body"` が 630px→632px にずれ失敗）、この場合は閾値による吸収が効かない。対策として、位置はロケーターの実測値・サイズは既知の正しい固定値としてクロップ撮影する（`expectClippedScreenshot`、`page.screenshot`/`toHaveScreenshot` の `clip` オプション）ことで、寸法の揺れをピクセル内容差として `maxDiffPixelRatio` の枠内に吸収できるようにした。
...
（旧 17.4/17.5 — 0.2.150 で採番修正。それ以前の改訂履歴の §17.5 参照は旧採番であり、当時の本節を指す）
...
**TreeGantt本体（`api`/`frontend`）はAIの存在を一切知らない。** AIは、人間がブラウザでTreeGanttを使うのと同じ立場の外部クライアントとして、既存のREST API（`/api/v1/*`）を叩くだけである。AI連携のために `api`/`frontend` へAI関連の変更・依存パッケージを加えることはしない。

方針転換の経緯・検討した代替案・却下理由・将来の拡張条件は `docs/ai_integration_policy.md` に詳細を記録する。本節は現行実装の要点のみを記す。
...
| 0.2.131 | 2026/7 | 製品バージョンを **1.3.2** に更新（ヘッダー・ステータス・構成図）。自動ゲート導入（CI・ESLint・`typecheck`/`lint` npm scripts・Docker healthcheck、0.2.128 で明記した設計）と、import/restore・CSV往復・タスク作成時の `titleColor`/`titleBgColor`/`estimateMinutes` 消失修正、reorder の循環・プロジェクト境界検証、バッチ作成の検証強化、タスク一覧 `limit` の範囲検証（0.2.129〜0.2.130 で明記した設計）を品質改善の製品リリースとして `CHANGELOG.md` の `[1.3.2]` に記録。 |
| 0.2.132 | 2026/7 | 500系エラーレスポンスのマスキング方針を追加（§11）。集約エラーハンドラは `statusCode>=500` のとき `error` を汎用文言 `Internal Server Error` に置き換えて内部詳細を返さず、詳細は Fastify logger にのみ出力する。4xx は従来どおり `err.message` を返す。 |
| 0.2.133 | 2026/7 | WebSocket のハードニング方針を追加（新設§6.4・§11）。`WebSocketServer` に `maxPayload`（64KB）を設定し過大メッセージの接続は自動切断する。接続時に `Origin` ヘッダーが存在し `CORS_ORIGIN` が `*` 以外のときのみオリジン検証を行い、不一致な接続を拒否する（`Origin` 無しの非ブラウザクライアントは従来どおり許可）。認証・認可自体は引き続き Phase 2 スコープ外。 |
| 0.2.134 | 2026/7 | CSV 式インジェクション対策を「将来課題」から実施済み仕様へ更新（§10・§11）。CSV Export のユーザー入力由来文字列列（`title`/`summary`/`description`/`assignee`）のうち先頭が `=+-@` のセルにシングルクォートを付与して無害化し、CSV Import で往復復元する方針を明記。 |
| 0.2.135 | 2026/7 | 本番 CORS の許可リスト化運用ガイドを §11 に明記。既定 `CORS_ORIGIN=*`（社内 LAN 運用の互換維持）は変更せず、インターネット公開等では具体オリジン1つを設定する運用を明文化（REST・WebSocket 双方に適用）。`.env.example` にコメント、`docker-compose.yml` の該当行にコメントを付与（コード変更なし）。 |
...
| 0.2.141 | 2026/7 | §9.9 のトースト対象一覧に、`GanttChart` の行 D&D（WBS 行のドラッグ&ドロップによる並び替え）失敗を追記。`App.tsx` が `reorderTasks` を `onReorder` に直接渡していたため、失敗時にロールバックはされるがエラートーストが表示されない欠落があった。`handleReorder`（他の CRUD ハンドラと同じ try/catch＋エラートーストのパターン）でラップする方針を明記。 |
| 0.2.142 | 2026/7 | 製品バージョンを **1.4.0** に更新（ヘッダー・ステータス・構成図・§15）。行D&D失敗時のエラートースト表示（0.2.141 で明記した設計）、E2E テスト2件の修正（現行UIの「＋ 子追加」フライアウトへの追従・日付依存の解消）を含む品質改善の製品リリースとして `CHANGELOG.md` の `[1.4.0]` に記録。 |
...
| 0.2.161 | 2026/7 | §9.2 に WBS/ガントの表示専用の通し番号列「No.」の新設を明記：既存「#」列（`task.seq`）の直前に配置し、`displayTasks` を `buildTree` → `flattenTree(roots, 空の Set)` で全展開・フィルタなしの順にフラット化した順序で 1 から採番。実際の画面表示（フィルタ・折りたたみ適用後）では、残った行にこの基準の元番号をそのまま表示し詰め直さない。クロスプロジェクト参照の合成グループ行・参照タスク本体も対象。`wbsHiddenCols`（`HIDEABLE_COLS`）には含めず常時表示。実装は次段階（テスト先行、コード変更なし）。 |
| 0.2.162 | 2026/7 | §9.2 の表示専用通し番号列の見出しを「No.」から「行」に変更（未リリースのため呼称のみの変更、0.2.161 時点の仕様自体に変更なし）。他の列見出し（タイトル・ST・優先・進捗・担当・開始・終了・日数）が日本語表記で統一されている点に揃え、既存「#」列（seq）との混同も避ける。将来の英語表示対応時の参考として英語表記 "Row" も本文に追記。 |
| 0.2.163 | 2026/7 | 新設§9.11「多言語対応（i18n）」：UI表示言語を日本語/英語で切り替え可能にする方針を明記。新規ライブラリは追加せず自前の軽量実装（`i18n/ja.ts`・`i18n/en.ts` を `satisfies` でキー同期・`useTranslation` フック・`taskStore` の `locale` 永続化・`App.tsx` の切替UI）。ユーザーが入力したデータは翻訳対象外、既定ロケール`'ja'`維持により既存テスト・既存ユーザーの見た目に影響なし。段階導入方針とし、第1弾（基盤・ツールバー・App.tsx・主要モーダル・WBS/ガント本体）のスコープを明記、コンテキストメニュー等は今後の対象として残す。 |
| 0.2.164 | 2026/7 | §9.11 の曜日ラベル一本化の記述を実装（基盤フェーズ、コミット待ちの実装内容）に合わせて是正：添字アクセスが必要なためフラットな `t()` 辞書ではなく専用モジュール `i18n/dow.ts`（`DOW_LABELS_JA`/`DOW_LABELS_EN` + `useDowLabels()`）に一本化する方式に修正。 |
| 0.2.165 | 2026/7 | 製品バージョンを **1.8.0** に更新（ヘッダー・ステータス・構成図・§15）。多言語対応（i18n）第1弾（基盤・ツールバー・App.tsx本体・TaskModal/MilestoneModal・WBS/ガント本体の列見出しを含む主要画面）を機能追加の製品リリースとして `CHANGELOG.md` の `[1.8.0]` に記録。列見出しの辞書一本化は表示専用（ヘッダー短縮形／列表示設定ポップアップ完全形）を区別し、表示文言自体は変更していない。**今回は major.minor が 1.7→1.8 に変わるマイナーバンプのため§15「現行リリース」も更新した**（データ形式版 1.1 は変更なし）。コンテキストメニュー・リソースビュー・プロジェクト管理・細かいトースト文言・APIエラーコード全件マッピングは第2弾として今後の対象のまま。 |
| 0.2.166 | 2026/7 | 製品バージョンを **1.9.0** に更新（ヘッダー・ステータス・構成図・§15）。多言語対応（i18n）第2弾（コンテキストメニュー・ダイアログ/トースト・リソースビュー・プロジェクト管理/クロスプロジェクト参照管理・hooks各所のトースト文言・APIエラーコードの主要マッピング、およびレビューで検出した見落とし5件＝WBSグリッドのステータス/優先度ラベル二重管理・ガントパネルのaria-label・今日ラインの表示/時刻ロケール・タイトル複製サフィックス・useImportExportのエラーメッセージの是正）を機能追加の製品リリースとして `CHANGELOG.md` の `[1.9.0]` に記録。§9.11 の段階導入方針を更新し、フロントエンドの静的UI文言はおおむね網羅した旨を明記。**今回は major.minor が 1.8→1.9 に変わるマイナーバンプのため§15「現行リリース」も更新した**（データ形式版 1.1 は変更なし）。 |
| 0.2.167 | 2026/7 | 製品バージョンを **1.9.1** に更新（ヘッダー・ステータス・構成図）。CI（1.9.0タグ）で発覚したビジュアルリグレッションのベースライン不整合（表示専用の通し番号列「行」追加後にガントメイン表示のベースラインを再生成していなかったための3%差分・閾値2%超過）の修正、README用スクリーンショット3点（ヒーロー画像の今日ラインとバーの重なり解消・依存関係/競合解決UIの新規撮影）、および設計書改訂履歴の古い昇順崩れ（0.2.90〜0.2.93）是正をパッチリリースとして `CHANGELOG.md` の `[1.9.1]` に記録。**パッチのみのバンプのため§15「現行リリース」は変更なし**。 |
| 0.2.168 | 2026/7 | 製品バージョンを **1.9.2** に更新（ヘッダー・ステータス・構成図）。§17.2/§14 にビジュアルリグレッションの寸法不一致対処（クロップ撮影, `expectClippedScreenshot`）を追記。CI（1.9.1タグ）で `gantt-chart-body` の高さが環境差（1280×630→1280×632）で寸法不一致となり `maxDiffPixelRatio` の評価前に即失敗する事象が発生したため、位置は実測値・サイズは既知の固定値としてクロップ撮影する方式に変更し、寸法の揺れをピクセル内容差として閾値の枠内に吸収できるようにした。3シナリオともローカル環境・CI相当のフォント環境（Noto Sans CJK JPのみ）で5回連続green確認。パッチリリースとして `CHANGELOG.md` の `[1.9.2]` に記録。**パッチのみのバンプのため§15「現行リリース」は変更なし**。 |
| 0.2.169 | 2026/7 | 製品バージョンを **1.9.3** に更新（ヘッダー・ステータス・構成図）。§17.2の対象範囲を3シナリオから2シナリオ（ガントメイン表示・TaskModal）に変更し、ツールバーを除外。CI（1.9.2タグ）でツールバーのみ寸法一致後もピクセル内容差4%（閾値2%超過）で継続的に失敗し、ローカルでフォントファイルを揃えて再現を試みても検出できなかった（フォントヒンティング等OS由来の差までは再現できないことが判明）ため、文字要素の密度が最も高くフォントレンダリング差の影響を受けやすいツールバーシナリオを対象から除外する判断とした。`toolbar-chromium-linux.png` のベースラインを削除。パッチリリースとして `CHANGELOG.md` の `[1.9.3]` に記録。**パッチのみのバンプのため§15「現行リリース」は変更なし**。 |
| 0.2.170 | 2026/7 | 製品バージョンを **1.9.4** に更新（ヘッダー・ステータス・構成図）。§17.2にTaskModalの閾値個別緩和（0.02→0.035）を追記。CI（1.9.3タグ）でTaskModalが1.9.0初出時と同一のピクセル数（9334px・約3%）で安定して超過し続けており、実際にはこの問題が一度も解消されていなかったことが判明（1.9.1でガントメイン表示のベースライン不整合を修正した際、TaskModalはローカルで通過していたため誤って「問題なし」と判断していた）。ツールバーと異なり超過幅が小さく一定しているため、除外ではなく`expectClippedScreenshot`呼び出し側での`maxDiffPixelRatio`個別指定（0.035）で吸収する方式とした。パッチリリースとして `CHANGELOG.md` の `[1.9.4]` に記録。**パッチのみのバンプのため§15「現行リリース」は変更なし**。 |
