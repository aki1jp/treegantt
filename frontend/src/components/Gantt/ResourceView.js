...
const HEADER_H = 22;
const CELL_H = 28;
export function ResourceView({ tasks, min, zoomLevel, totalWidth, labelWidth, scrollRef, }) {
    const { dayWidth } = ZOOM_CONFIG[zoomLevel];
    const totalDays = Math.round(totalWidth / dayWidth);
    // 担当者ごとにタスクをグループ化
    const assigneeGroups = new Map();
    for (const t of tasks) {
        if (!t.assignee || !t.startDate)
            continue;
        if (!assigneeGroups.has(t.assignee))
            assigneeGroups.set(t.assignee, []);
    ...
    if (assignees.length === 0)
        return null;
    // 各日の文字列（YYYY-MM-DD）
    const dayStrings = Array.from({ length: totalDays }, (_, i) => dayjs(min).add(i, 'day').format('YYYY-MM-DD'));
    return (_jsxs("div", { "data-testid": "workload-panel", style: {
            ...
            background: 'var(--th-bg)',
                    ...
                    display: 'flex', flexDirection: 'column',
                        ...
                        }, children: "\u30EA\u30BD\u30FC\u30B9\u30D3\u30E5\u30FC\uFF08\u62C5\u5F53\u8005\u5225\u8CA0\u8377\uFF09" }), assignees.map((a, ai) => (_jsx("div", { style: {
                        ...
                        }, children: a }, a)))] }), _jsx("div", { ref: scrollRef, style: { flex: 1, overflowX: 'hidden', overflowY: 'hidden' }, children: _jsxs("div", { style: { width: totalWidth, display: 'flex', flexDirection: 'column' }, children: [_jsx("div", { style: {
                                height: HEADER_H, position: 'relative', flexShrink: 0,
                                background: 'var(--th-bg2)', borderBottom: '1px solid var(--th-border)',
                            }, children: dayStrings.map((_, i) => {
                            ...
                            return (_jsx("div", { style: {
                                ...
                                }, children: dayStrings.map((dayStr, di) => {
                                    const dow = dayjs(min).add(di, 'day').day();
                                    const isSat = dow === 6;
                                    const isSun = dow === 0;
                                    const count = rowTasks.filter(t => t.startDate <= dayStr && (t.endDate ?? t.startDate) >= dayStr).length;
                                    ...
                                    return (_jsx("div", { style: {
                                        ...
                                        }, title: count > 0 ? `${a} ${dayStr}: ${count}件` : undefined, children: dayWidth >= 10 && count > 0 ? count : '' }, di));
                                }) }, a));
                        })] }) })] }));
}
