...
import { dateToX, calcGanttRange, calcLightningPoints, calcNowX, ganttTotalWidth, } from '../utils/ganttCalc';
import { buildTree, flattenTree, calcEffectiveProgress, buildChildCountMap, includeAncestors, } from '../utils/taskTree';
import { importFromJson, importFromCsv, exportToJson, exportToCsv, } from '../utils/importExport';
...
describe('フロントエンド 悪意テスト — ganttCalc 境界値・不正入力', () => {
    ...
    it('全タスクが done のとき全点が nowX になる', () => {
        ...
        const pts = calcLightningPoints(tasks.map(t => ({ task: t, effectiveProgress: 100 })), MIN, 'day');
    ...
    it('全タスクが wait のとき全点が nowX になる', () => {
        ...
        const pts = calcLightningPoints(tasks.map(t => ({ task: t, effectiveProgress: 0 })), MIN, 'day');
        pts.forEach(p => expect(p.x).toBe(nowX));
    });
    it('1000 件タスクでも ganttTotalWidth はクラッシュしない', () => {
        const tasks = Array.from({ length: 1000 }, (_, i) => makeTask({ id: `t${i}`, startDate: '2020-01-01', endDate: '2030-12-31' }));
...
describe('フロントエンド 悪意テスト — taskTree 循環・異常構造', () => {
    ...
    it('存在しない parentId は root 扱いになる', () => {
        const tasks = [makeTask({ id: 'A', parentId: 'ghost-parent' })];
        const { roots } = buildTree(tasks);
        expect(roots).toHaveLength(1);
        expect(roots[0].task.id).toBe('A');
    });
    it('50 段ネストのツリーを flattenTree しても全件取得できる', () => {
        const tasks = Array.from({ length: 50 }, (_, i) => makeTask({ id: `t${i}`, parentId: i > 0 ? `t${i - 1}` : null }));
        const { roots } = buildTree(tasks);
    ...
    it('filtered の parentId が all に存在しないとき安全に打ち切る（L60 break）', () => {
        const taskA = makeTask({ id: 'A', parentId: 'ghost-not-in-all' });
        // all に parentId が存在しない → L60 の break が発動する
        const result = includeAncestors([taskA], [taskA]);
        expect(result).toHaveLength(1); // A だけ（ゴースト親は追加されない）
        expect(result[0].id).toBe('A');
    });
    it('深さ 1000 の calcEffectiveProgress は葉ノードに到達できる', () => {
        const tasks = Array.from({ length: 10 }, (_, i) => makeTask({ id: `t${i}`, parentId: i > 0 ? `t${i - 1}` : null, progress: 80 }));
        const childCount = buildChildCountMap(tasks);
...
describe('フロントエンド 悪意テスト — 長大タイトル・特殊文字の堅牢性', () => {
    ...
    it('1000件のタスクを buildTree してもクラッシュしない', () => {
        const tasks = Array.from({ length: 1000 }, (_, i) => makeTask({ id: `t${i}`, parentId: i > 0 ? `t${Math.floor(i / 2)}` : null }));
        expect(() => buildTree(tasks)).not.toThrow();
        const { roots } = buildTree(tasks);
        const flat = flattenTree(roots, new Set());
