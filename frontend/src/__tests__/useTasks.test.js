...
describe('updateTask', () => {
    it('ストアを楽観的に更新し API を呼ぶ', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/tasks/t1'), expect.objectContaining({ method: 'PATCH' }));
    });
...
describe('deleteTask', () => {
    it('ストアからタスクを削除し API を呼ぶ', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/tasks/t1'), expect.objectContaining({ method: 'DELETE' }));
    });
});
...
describe('reorderTasks', () => {
    it('ストアの order を更新し API を呼ぶ', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith(expect.stringContaining('/tasks/reorder'), expect.objectContaining({ method: 'PATCH' }));
    });
    it('fetch 失敗時にストアを元の状態にロールバックする', async () => {
        const t1 = makeTask({ id: 't1', order: 1 });
        const t2 = makeTask({ id: 't2', order: 2 });
