...
function renderDialog(overrides = {}) {
    return render(_jsx(ConflictDialog, { field: "title", theirVal: "\u30EA\u30E2\u30FC\u30C8\u306E\u5909\u66F4", myVal: "\u81EA\u5206\u306E\u5909\u66F4", onResolve: NOOP, ...overrides }));
}
// ── CSS 変数の使用検証 ─────────────────────────────────────────────────────────
...
describe('ConflictDialog ハードコード色の排除', () => {
    it('ブロック背景にライト専用色 #fef3c7 が含まれない', () => {
        renderDialog();
        const block = screen.getByText('別のユーザーの変更').closest('div[style]');
        expect(block?.style.background).not.toContain('#fef3c7');
    });
    it('ブロック背景にライト専用色 #eff6ff が含まれない', () => {
        renderDialog();
        const block = screen.getByText('あなたの変更').closest('div[style]');
