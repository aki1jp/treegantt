...
describe('GanttChart スクロール分離レイアウト', () => {
    function renderChart() {
        return render(_jsx(GanttChart, { onEditTask: NOOP, onDeleteTask: NOOP, onInlineUpdate: NOOP, onQuickAdd: NOOP, onAddSubTask: NOOP, onReorder: NOOP }));
    }
    ...
    it('ガントパネルは overflow: auto（横スクロールバーあり）', () => {
        const { getByTestId } = renderChart();
        const gantt = getByTestId('gantt-panel');
    ...
    it('ガントヘッダーの内行すべてに boxSizing:border-box が設定されている', () => {
        ...
        const innerRows = Array.from(ganttHeader.children);
    ...
    it('WBSパネル上でホイール操作すると WBS ボディの scrollTop も同期される', () => {
        ...
        const wbsBody = container.querySelector('[data-testid="wbs-panel"] > div:last-child');
...
describe('親タスク WBS日付セル 読み取り専用スタイル', () => {
    ...
    it('親タスク日付セルの cursor は "default"', () => {
        const { container } = renderWithParentChild();
        const cell = container.querySelector('[data-testid="date-readonly"]');
...
describe('WBS 行D&D — インライン編集中のテキスト選択干渉防止', () => {
    ...
    it('INPUTにフォーカスがある状態でdragstartはpreventDefaultされる（行D&Dが発動しない）', () => {
        ...
        const rowWrapper = wbsPanel.querySelector('[draggable="true"]');
...
describe('WBS 行D&D — マウス位置によるインデント深さ選択', () => {
    ...
    function renderWith3Tasks(onReorder = vi.fn()) {
        ...
        return render(_jsx(GanttChart, { onEditTask: NOOP, onDeleteTask: NOOP, onInlineUpdate: NOOP, onQuickAdd: NOOP, onAddSubTask: NOOP, onReorder: onReorder }));
    }
    ...
    it('dragOver 時のインジケーター left が clientX に連動した depth を反映する', () => {
        ...
        fireEvent.dragStart(rows[0]);
        fireEvent(rows[2], new MouseEvent('dragover', { bubbles: true, cancelable: true, clientX: 77 }));
        const line = wbsPanel.querySelector('[data-drop-line]');
    ...
    function makeFourTasks() {
        const base = {
            summary: '', description: '', status: 'todo', priority: 'medium',
    ...
    // ── 子採用モード ──────────────────────────────────────────────────────────
    function mockRowRect(row, height = 32) {
    ...
    it('adopt zone: 行下端70%にホバーするとハイライト・バーなし', () => {
        ...
        expect(wbsPanel.querySelector('[data-drop-line]')).toBeNull();
        expect(rows[1].style.outline).toContain('#4f46e5');
        vi.restoreAllMocks();
    });
    ...
    it('adopt zone: 自分自身の行はハイライトされず onReorder も呼ばれない', () => {
        ...
        expect(rows[0].style.outline).toBeFalsy();
    ...
    it('adopt zone: マイルストーンはハイライトされずバー挿入にフォールバック', () => {
        ...
        const { getByTestId } = render(_jsx(GanttChart, { onEditTask: NOOP, onDeleteTask: NOOP, onInlineUpdate: NOOP, onQuickAdd: NOOP, onAddSubTask: NOOP, onReorder: vi.fn() }));
        const wbsPanel = getByTestId('wbs-panel');
        const rows = wbsPanel.querySelectorAll('[draggable="true"]');
        ...
        expect(rows[2].style.outline).toBeFalsy(); // ハイライトなし
...
describe('WBS 日付インライン編集 — blur で確定', () => {
    ...
    function renderEditChart() {
        return render(_jsx(GanttChart, { onEditTask: NOOP, onDeleteTask: NOOP, onInlineUpdate: onInlineUpdate, onQuickAdd: NOOP, onAddSubTask: NOOP, onReorder: NOOP }));
    }
    it('開始日セルをクリックすると date input が表示される', () => {
        const { container } = renderEditChart();
        const wbsPanel = container.querySelector('[data-testid="wbs-panel"]');
