...
describe('ProjectTabs — バツボタンが表示されない', () => {
    it('✕ ボタンが DOM に存在しない', () => {
        ...
        render(_jsx(ProjectTabs, { projects: projects, currentProject: projects[0], onSelect: NOOP, onDelete: NOOP, onRename: NOOP }));
        // ✕ / × の文字を持つボタンがない
...
describe('ProjectTabs — 右クリックでコンテキストメニューが表示される', () => {
    ...
    it('コンテキストメニューの削除をクリックすると onDelete が該当プロジェクトで呼ばれる', () => {
        ...
        const projects = [makeProject('p1', 'Alpha'), makeProject('p2', 'Beta')];
        render(_jsx(ProjectTabs, { projects: projects, currentProject: projects[0], onSelect: NOOP, onDelete: onDelete }));
    ...
    it('「名前を変更」をクリックすると onRename が呼ばれる', () => {
        ...
        const projects = [makeProject('p1', 'Alpha')];
        render(_jsx(ProjectTabs, { projects: projects, currentProject: projects[0], onSelect: NOOP, onDelete: NOOP, onRename: onRename }));
        fireEvent.contextMenu(screen.getByText('Alpha'));
    ...
    it('mousedown でコンテキストメニューが閉じる', () => {
        const projects = [makeProject('p1', 'Alpha')];
        const { container } = render(_jsx(ProjectTabs, { projects: projects, currentProject: projects[0], onSelect: NOOP, onDelete: NOOP, onRename: NOOP }));
        fireEvent.contextMenu(screen.getByText('Alpha'));
        expect(screen.getByText('削除')).toBeTruthy();
