...
describe('importFromCsv — 複数タスク・意地悪テスト', () => {
    ...
    it('100件のタスクが全件パースされる', () => {
        const manyTasks = Array.from({ length: 100 }, (_, i) => makeTask({ id: `t${i}`, title: `Task ${i}`, seq: i + 1, order: i + 1 }));
...
describe('importFromJson — 意地悪テスト', () => {
    ...
    it('100件のラウンドトリップが全件復元できる', () => {
        const tasks = Array.from({ length: 100 }, (_, i) => makeTask({ id: `task-${i}`, title: `Task ${i}`, seq: i + 1, order: i + 1 }));
