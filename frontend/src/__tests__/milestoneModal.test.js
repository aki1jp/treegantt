...
describe('MilestoneModal — backdrop クリックの閉じる/閉じない挙動', () => {
    it('既存マイルストーン・変更なし: backdrop クリックで onClose が呼ばれる', () => {
        const onClose = vi.fn();
        const { container } = render(_jsx(MilestoneModal, { task: makeMilestone(), allTasks: [], onSave: NOOP, onClose: onClose }));
        fireEvent.click(container.firstChild);
        expect(onClose).toHaveBeenCalledOnce();
    });
    it('新規マイルストーン・変更なし: backdrop クリックで onClose が呼ばれる', () => {
        const onClose = vi.fn();
        const { container } = render(_jsx(MilestoneModal, { task: null, allTasks: [], onSave: NOOP, onClose: onClose }));
        fireEvent.click(container.firstChild);
        expect(onClose).toHaveBeenCalledOnce();
    });
    it('タイトル変更後: backdrop クリックで onClose が呼ばれない', () => {
        const onClose = vi.fn();
        const { container } = render(_jsx(MilestoneModal, { task: makeMilestone({ title: '元タイトル' }), allTasks: [], onSave: NOOP, onClose: onClose }));
    ...
    it('担当者変更後: backdrop クリックで onClose が呼ばれない', () => {
        const onClose = vi.fn();
        const { container } = render(_jsx(MilestoneModal, { task: makeMilestone({ assignee: '' }), allTasks: [], onSave: NOOP, onClose: onClose }));
...
describe('MilestoneModal — backdrop クリック時の shake アニメーション', () => {
    ...
    it('タイトルのみ変更 → 未変更フィールド（担当者）は shake しない', () => {
        const { container } = render(_jsx(MilestoneModal, { task: makeMilestone(), allTasks: [], onSave: NOOP, onClose: vi.fn() }));
        fireEvent.change(screen.getAllByRole('textbox')[0], { target: { value: '変更後' } });
        fireEvent.click(container.firstChild);
