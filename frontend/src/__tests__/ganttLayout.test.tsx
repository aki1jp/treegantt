...
describe('ガントヘッダー 曜日表示', () => {
  function renderDayZoom() {
    ...
    return render(
      <GanttChart onEditTask={NOOP} onDeleteTask={NOOP} onInlineUpdate={NOOP} onQuickAdd={NOOP} onAddSubTask={NOOP} onReorder={NOOP} onCopyInsert={NOOP} />
    );
  }
...
describe('WBS 行D&D — マウス位置によるインデント深さ選択', () => {
  ...
  function renderWith3Tasks(onReorder = vi.fn()) {
    ...
    return render(
      <GanttChart onEditTask={NOOP} onDeleteTask={NOOP} onInlineUpdate={NOOP}
        onQuickAdd={NOOP} onAddSubTask={NOOP} onReorder={onReorder} onCopyInsert={NOOP} />
    );
  }
  ...
  it('adopt zone: マイルストーンはハイライトされずバー挿入にフォールバック', () => {
    ...
    const { getByTestId } = render(
      <GanttChart onEditTask={NOOP} onDeleteTask={NOOP} onInlineUpdate={NOOP}
        onQuickAdd={NOOP} onAddSubTask={NOOP} onReorder={vi.fn()} onCopyInsert={NOOP} />
    );
    const wbsPanel = getByTestId('wbs-panel');
    const rows = wbsPanel.querySelectorAll('[draggable="true"]');
...
describe('マイルストーン強調 UI', () => {
  const MILESTONE_TASK = {
    id: 'ms1', projectId: 'p1', parentId: null,
    title: 'リリース', summary: '', description: '',
    status: 'todo' as const, priority: 'medium' as const, progress: 0, assignee: '',
...
// ─── i18n（locale='en'）: ガントパネルの aria-label ──────────────────────
describe('GanttChart i18n（locale="en"）— ガントパネル aria-label', () => {
  function renderChart() {
    return render(
      <GanttChart
        onEditTask={NOOP}
  ...
  it('ガントパネルの aria-label が英語表示される（Gantt Chart）', () => {
    const { getByTestId } = renderChart();
    expect(getByTestId('gantt-panel').getAttribute('aria-label')).toBe('Gantt Chart');
  });
});

describe('GanttChart 既定（locale="ja"）— ガントパネル aria-label', () => {
  function renderChart() {
    return render(
      <GanttChart
        onEditTask={NOOP}
  ...
  it('ガントパネルの aria-label は既定で「ガントチャート」', () => {
    const { getByTestId } = renderChart();
    expect(getByTestId('gantt-panel').getAttribute('aria-label')).toBe('ガントチャート');
  });
});
