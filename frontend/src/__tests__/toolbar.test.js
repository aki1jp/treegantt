...
function renderToolbar() {
    return render(_jsx(Toolbar, { onAddTask: NOOP, onAddMilestone: NOOP, onImport: NOOP, onRestore: NOOP, onExportJson: NOOP, onExportCsv: NOOP }));
}
beforeEach(() => {
    localStorage.clear();
...
describe('Toolbar フィルタインライン表示', () => {
    it('行2（展開時）にステータス選択が直接表示される', () => {
        ...
        const statusSelect = Array.from(selects).find(s => Array.from(s.options).some(o => o.text === 'TODO'));
    ...
    it('行2（展開時）に優先度選択が直接表示される', () => {
        ...
        const row2 = screen.getByTestId('toolbar-row2');
        const selects = row2.querySelectorAll('select');
        const prioritySelect = Array.from(selects).find(s => Array.from(s.options).some(o => o.text === '最高'));
