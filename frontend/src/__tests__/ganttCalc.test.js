...
describe('calcLightningPoints', () => {
    ...
    const zoom = 'day';
    const dayWidth = ZOOM_CONFIG['day'].dayWidth;
    function makeRow(status, progress) {
        return {
            task: makeTask({
                status,
                progress,
    ...
    it('pending タスクはイナズマラインでスキップされる', () => {
        const rows = [makeRow('pending', 0)];
        const pts = calcLightningPoints(rows, minDate, zoom);
        expect(pts).toBeNull();
    });
    it('pending と wip が混在する場合、pending はスキップされ wip のみ点が生成される', () => {
        const rows = [makeRow('pending', 0), makeRow('wip', 50)];
        const pts = calcLightningPoints(rows, minDate, zoom);
        expect(pts).toHaveLength(1);
        const expectedWipX = Math.round((10 * dayWidth + dayWidth) * 0.5);
        expect(pts[0].x).toBe(expectedWipX);
    });
});
...
describe('日付座標系の一貫性', () => {
    ...
    it('calcDuration: ローカル日付差が正しく計算される', () => {
        const task = { startDate: '2026-05-20', endDate: '2026-05-22' };
        // 3日間（20,21,22）
