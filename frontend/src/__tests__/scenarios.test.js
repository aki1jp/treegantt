...
import { calcGanttRange, calcLightningPoints, calcCriticalPath, calcDuration, calcTodayX, calcNowX, ganttTotalWidth, dateToX, PERIOD_DAYS, ZOOM_CONFIG, ROW_HEIGHT_PX, } from '../utils/ganttCalc';
import { exportToJson, importFromJson, exportToCsv, importFromCsv, } from '../utils/importExport';
import { buildChildCountMap, buildTree, flattenTree, calcEffectiveProgress, includeAncestors, } from '../utils/taskTree';
...
describe('§3 タスクデータ項目', () => {
    it('ステータス値は todo / wip / done / wait の4種類', () => {
        const statuses = ['todo', 'wip', 'done', 'wait'];
        for (const s of statuses)
            expect(makeTask({ status: s }).status).toBe(s);
    });
    it('優先度値は critical / high / medium / low の4種類', () => {
        const priorities = ['critical', 'high', 'medium', 'low'];
        for (const p of priorities)
            expect(makeTask({ priority: p }).priority).toBe(p);
    });
...
describe('§4.3 ズームレベル', () => {
    it('day / week / month の3段階が存在する', () => {
        ...
        for (const l of levels)
            expect(ZOOM_CONFIG[l]).toBeDefined();
    });
...
describe('§4.8 イナズマライン (calcLightningPoints)', () => {
    const minDate = new Date('2026-01-01');
    function makeRow(task, effectiveProgress = task.progress) {
...
describe('§5.5 ツリー構造・折りたたみ', () => {
    ...
    describe('flattenTree', () => {
        ...
        it('ルートタスクのみ depth === 0（中間親は depth > 0）', () => {
            ...
            const isRootParent = (row) => row.depth === 0 && (childCount.get(row.task.id) ?? 0) > 0;
...
describe('WBS インデント計算', () => {
    ...
    const textStart = (depth) => BASE_PADDING + depth * INDENT_PER_LEVEL + ICON_WIDTH + GAP;
    ...
    it('アイコン種別の選択ロジックが正しい', () => {
        const iconType = (hasChildren, depth) => {
            if (hasChildren)
                return 'collapse'; // ▼/▶ ボタン
            if (depth > 0)
                return 'leaf-line'; // └ 記号
...
describe('列幅リサイズ — 最小幅制約', () => {
    ...
    const clamp = (key, w) => Math.max(COL_MIN_WIDTHS[key] ?? 40, w);
...
describe('§8 Import / Export', () => {
    ...
    describe('JSON エクスポート', () => {
        ...
        it('predecessors 配列がそのまま保持される', () => {
            const data = JSON.parse(exportToJson(project, sampleTasks()));
            const tc = data.tasks.find((t) => t.id === 'task-c');
            expect(tc.predecessors).toEqual(['task-a', 'task-b']);
        });
        it('null の日付フィールドが保持される', () => {
            const data = JSON.parse(exportToJson(project, sampleTasks()));
            const td = data.tasks.find((t) => t.id === 'task-d');
...
describe('マイルストーン禁止ルール', () => {
    ...
    const parentCandidates = (tasks, selfId) => tasks.filter(t => t.id !== selfId && !t.isMilestone);
    ...
    const canAddChild = (task) => !task.isMilestone;
    ...
    const buildMilestonePayload = (title, date) => ({
        title,
        isMilestone: true,
        startDate: date || null,
        endDate: date || null,
