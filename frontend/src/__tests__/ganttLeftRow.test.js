...
function rowProps(task, onInlineUpdate) {
    return {
        task,
        depth: 0,
        hasChildren: false,
...
function renderRow(task, onInlineUpdate = vi.fn()) {
    return render(_jsx(GanttLeftRow, { ...rowProps(task, onInlineUpdate) }));
}
// ─── 通常編集（コンフリクトなし）──────────────────────────────────────────────
...
describe('GanttLeftRow インライン編集 — コンフリクト検出', () => {
    it('編集開始後にタスクが外部更新されると onInlineUpdate を呼ばずコンフリクトを表示する', () => {
        ...
        rerender(_jsx(GanttLeftRow, { ...rowProps(makeTask({ title: 'リモートの変更' }), onInlineUpdate) }));
        // ユーザーが blur（コミット試行）
        fireEvent.blur(input);
    ...
    it('外部更新が同じ値の場合はコンフリクトにならず正常コミットされる', () => {
        ...
        rerender(_jsx(GanttLeftRow, { ...rowProps(makeTask({ title: '元のタイトル' }), onInlineUpdate) }));
        fireEvent.blur(input);
        expect(onInlineUpdate).toHaveBeenCalledWith('t1', { title: '変更後' });
        expect(screen.queryByText('⚠️ 編集中に別のユーザーが変更しました')).toBeNull();
    });
...
describe('GanttLeftRow タイトルホバーツールチップ', () => {
    ...
    it('isDragging=true のときはマウスオーバーしてもツールチップが表示されない', () => {
        vi.useFakeTimers();
        render(_jsx(GanttLeftRow, { ...rowProps(makeTask({ summary: 'ホバーサマリ' }), vi.fn()), isDragging: true }));
        fireEvent.mouseEnter(screen.getByText('元のタイトル'), { clientX: 100, clientY: 100 });
        act(() => { vi.advanceTimersByTime(250); });
        expect(screen.queryByText('ホバーサマリ')).toBeNull();
    });
