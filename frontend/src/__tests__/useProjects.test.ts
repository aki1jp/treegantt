...
describe('useProjects — 多言語対応（locale: en）', () => {
  afterEach(() => {
    useTaskStore.setState({ locale: 'ja' });
  });

  it('初回ロード失敗時、error とトーストが英語表示になる', async () => {
    useTaskStore.setState({ locale: 'en' });
    vi.mocked(fetch).mockRejectedValue(new Error('network down'));

    const { result } = renderHook(() => useProjects());
    await act(async () => {});
    expect(result.current.error).toBe('Failed to fetch project list');
    const toasts = useToastStore.getState().toasts;
    expect(toasts.some(t => t.message.includes('Failed to fetch project list: network down'))).toBe(true);
  });
});
