...
export const hocuspocus = Server.configure({
  ...
  // Y.js ドキュメントが空（初回 or SQLiteデータなし）のとき tasks テーブルで初期化。
  ...
  // Y.js の変更を tasks テーブルに完全同期（upsert + 削除）
  async onStoreDocument({ document, documentName }) {
    ...
    const dbIds = new Set(
      (db.prepare('SELECT id FROM tasks WHERE project_id = ?').all(documentName) as { id: string }[])
        .map(r => r.id)
    );
    ...
    db.transaction(() => {
      for (const row of rows) {
        upsert.run(
          row.id, row.projectId, row.parentId,
      ...
      for (const dbId of dbIds) {
        if (!yjsIds.has(dbId)) deleteStmt.run(dbId);
      }
    })();
  },
});
